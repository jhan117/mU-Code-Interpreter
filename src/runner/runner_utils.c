...
void initSnapshot() {
  ...
  snapshot.memory[INIT_MEMORY_SIZE];
  snapshot.cpu_stack[INIT_CPU_STACK_CAPACITY];
  snapshot.cpu_top = 0;
  snapshot.cs = ctx->cs;
  snapshot.pc = ctx->pc;
  snapshot.ds = ctx->ds;
...
void expandChangeList() {
  ...
  ctx->changes.change_list =
      realloc(ctx->changes.change_list, ctx->changes.list_size);
}
...
void saveChanges() {
  ...
  for (int i = start; i <= end; i++) {
    if (snapshot.cpu_stack[i] != ctx->cpu_stack.items[i]) {
      addNewChange(HD_NUM_CPU_STACK, i, ctx->memory[i]);
    }
  }
  // 레지스터
